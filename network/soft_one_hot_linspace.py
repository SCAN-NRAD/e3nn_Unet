...
def plot():
        ...
        fig, axss = plt.subplots(len(bases), 4, figsize=(9, 6), sharex=True, sharey=False)

        for axs, b in zip(axss, bases):
            for ax, c in zip(axs, [True, False, 'left', 'right']):
                plt.sca(ax)
                plt.plot(x, soft_one_hot_linspace(x, -0.5, 1.5, number=5, basis=b, cutoff =c))
                if b != 'bessel':
                    plt.plot([-0.5]*2, [-2, 2], 'k-.')
                    plt.plot([1.5]*2, [-2, 2], 'k-.')
                    ax.set_ylim(-1, 1.5)
                else:
                    plt.plot([-0.5]*2, list(ax.get_ylim()), 'k-')
                    plt.plot([1.5]*2, list(ax.get_ylim()), 'k-')
                    #print(list(ax.get_ylim()))
                plt.title(f"{b}" + (f" with\ncutoff {c}" if c else ""))
